...
def search_node(state: ResearchState) -> ResearchState:
    """Search corpus + optional live Wikipedia augmentation."""
    query = state["query"]
    ...
    try:
        ...
        if wiki.get("content"):
            wiki_id = f"wiki_{hash(query) % 10_000}"
            if wiki_id not in seen_ids:
        ...
        logger.warning("wikipedia fetch skipped: %s", exc)
...
def critic_node(state: ResearchState) -> ResearchState:
    """Validate evidence coverage and adjust confidence before drafting."""
    ...
    for q in questions:
        q_tokens = set(_tokenise(q))
        covered = any(
            q_tokens & set(_tokenise(d["title"] + " " + " ".join(d.get("topics", []))))
            for d in results
        )
        if not covered:
            unsupported.append(q)

    penalty = min(0.25, 0.05 * len(unsupported))
    confidence = max(0.0, float(state.get("confidence", 0.0)) - penalty)
    key_facts = list(state.get("key_facts", []))
    if unsupported:
        key_facts.append(
            f"Critic noted {len(unsupported)} research questions without direct source coverage."
        )

    return ResearchState(
        **{
            **state,
            "gaps": gaps + unsupported,
...
def draft_node(state: ResearchState) -> ResearchState:
    ...
    try:
        from agent_core import complete_text, llm_available, write_run_artifacts

        if llm_available() and key_findings:
            synthesis = complete_text(
                "Synthesize these research findings into 3 bullet points with citations:\n"
                + "\n".join(key_findings[:6]),
                system="You are a research analyst. Be factual and cite sources inline.",
            )
            key_findings = [synthesis]
    except Exception as exc:
        logger.warning("draft enrichment skipped: %s", exc)
    ...
    try:
        from agent_core import write_run_artifacts

        write_run_artifacts(
            project="langgraph-research-analyst",
            summary={
                "query": query,
                "confidence": confidence,
            ...
            trace_events=[{"node": "draft", "confidence": confidence}],
        )
    except Exception:
        pass
...
def merge_search_node(state: ResearchState) -> ResearchState:
    """Deduplicate and rank evidence from parallel researchers."""
    from agent_core import merge_ranked_results

    merged = merge_ranked_results(state, results_key="search_results", limit=10)
    iteration = state.get("search_iterations", 0) + 1
    return ResearchState(**{**state, **merged, "search_iterations": iteration})
...
def build_graph():
    ...
    workflow.add_node("corpus_researcher", corpus_researcher_node)
    workflow.add_node("web_researcher", web_researcher_node)
    workflow.add_node("merge_search", merge_search_node)
    ...
    workflow.add_edge("plan", "corpus_researcher")
    workflow.add_edge("corpus_researcher", "web_researcher")
    workflow.add_edge("web_researcher", "merge_search")
    workflow.add_edge("merge_search", "analyze")
    ...
    workflow.add_conditional_edges(
        "review",
        _should_continue,
        {"search": "corpus_researcher", "end": END},
    )
